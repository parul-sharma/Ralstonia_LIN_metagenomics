...
def  main():
	...
	#print(type(data.loc[0, 'taxid_LIN']))
	data['taxid_LIN'] = data['taxid_LIN'].str.strip('[]')
	data['parent_LIN'] = data['parent_LIN'].str.strip('[]')
	#print(data)

	in_file_orig = args.in_file_report
        #reading the file
	in_file_orig = pd.read_csv(in_file_orig,sep='\t',header=None,index_col=False,skiprows=3)
	in_file = in_file_orig.iloc[:,[0,1,2,3,6]]
	in_file.columns = range(in_file.columns.size)
